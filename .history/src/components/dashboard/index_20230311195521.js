...
function Dashboard(props) {
    ...
    }, [fetchData, gameData,activeGames]);
    ...
    const handleCreateGame = () => {
        ...
        GameService.createGame({}).then(data => {
            if (data) {
                ...
                navigate('/game', {
                    state:  {game:data.game,userId:user.id}
                });
            }
        }).catch(error => {
            console.log(error);
    ...
    const GamesTableRow = (props) => {
        return <tr>
            ...
            <td style={{ backgroundColor: "white", cursor: 'pointer' }} onClick={() => handleJoinRoom(props.count-1)}>
                <h5>{props.room}</h5>
            </td>
            <td>
