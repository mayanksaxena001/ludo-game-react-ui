...
function Home() {
  ...
  const [boxColor,setBoxColor] = useState('background-color:'+COLORS[0]);
  useEffect(()=>{

    let id = setInterval(()=>{
      setBoxColor(boxColor=>'background-color:'+COLORS[randomIntFromInterval(0,2)]);
    },30);
    return ()=>{
      clearInterval(id);
    }
  ...
  return (
    <div className="App">
      ...
      <div className='home-box'>
        <div style={{boxColor}} className={toggleActiveStyle(1)} onClick={() => goToHome()}>Home</div>
