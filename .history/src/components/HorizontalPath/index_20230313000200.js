...
function HorizontalPath(props) {
    ...
    const map = new Map();
    for (let i = 1; i <= 18; i++) {
        map.set(i, <PathBox handleTokenMove={handleTokenMove} id={i} value={id} minheight={height} minWidth={width} />)
    }
    if (id === '4') {
    ...
    COLORED_BOXES_ID.map(boxId => {
        // if(boxId ===)
        map.set(boxId, <PathBox handleTokenMove={handleTokenMove} id={boxId} disabled={true} color={color} value={id} minheight={height} minWidth={width} />);
    });
