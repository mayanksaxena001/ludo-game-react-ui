...
function LudoGame(props) {
    ...
    const receivedMessage = useCallback((data) => {
        if (data) {
            ...
            if (!ispresent) {
                ...
                let player_ = addPlayer(indexCount, data, user);
                setPlayer(player_);
            }
            if (data.has_started) {
    ...
    useEffect(() => {
        ...
        let player_ = addPlayer(player_turn, gameData, user);
        setPlayer(player_);
    ...
    const addPlayer = (indexCount, data, user) => {
        let player_ = Player;
        player_.id = user.id;
        ...
        let tokenCount = data.token_count;
        if (tokenCount) {
            for (var i = 0; i < tokenCount; i++) {
                var token = Token;
                let value=i;
                token.id = indexCount + ':' + (value+1);
