...
function LudoGame(props) {
    ...
    let color = ['#ff0000', '#0000ff', '#008000', '#ffff00'];
    ...
    const receivedMessage = useCallback((data) => {
        if (data) {
            ...
            if (!ispresent) {
                ...
                player_.house.tokens = [indexCount + '1', indexCount + '2', indexCount + '3', indexCount + '4'];
                player_.color = color[indexCount];
                setPlayer(player_);
            }
            if (data.has_started) {
    ...
    useEffect(() => {
        ...
        player_.house.tokens = [player_turn + '1', player_turn + '2', player_turn + '3', player_turn + '4'];
        player_.username = user.username;
        player_.house.id = playerTurn;
        player_.color = color[player_turn];
        setPlayer(player_);
    ...
    if (!gameStarted) {
        ...
        return (
            <SocketChannelContext.Provider value={socket}>
                <GameBoard room={room} id={game.id} player={player} playerTurn={playerTurn} handleTokenMove={handleTokenMove} disabled={isDisabled} gameData={gameData} dicehandler={dicehandler} />
