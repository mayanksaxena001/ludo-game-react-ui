...
import socketService from '../../service/socket.service';
import addPlayer from './ludo.logic';
...
function LudoGame(props) {
    ...
    const receivedMessage = useCallback((data) => {
        if (data) {
            ...
            if (!ispresent) {
                ...
                let player_ = addPlayer(indexCount, data.token_count, user, COLORS);
    ...
    useEffect(() => {
        ...
        let player_ = addPlayer(player_turn, gameData.token_count, user, COLORS);
