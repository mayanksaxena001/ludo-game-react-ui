...
function LudoGame(props) {
    ...
    const receivedMessage = useCallback((data) => {
        if (data) {
            ...
            if (data.has_started && playerCount == data.player_count) {
    ...
    const startGameHandler = () => {
        //TODO start game / disable start button when all players joined
        setGameStarted(true);
    ...
    if (!gameStarted) {
        // const startDisabled = (game.game.created_by != userId ? true : false) && !gameStarted;
        return (<>
            <h1>Waiting for Players to join</h1>
            <button disabled={gameStarted} onClick={startGameHandler}>Start Game</button>
        </>
        )
    }
    else
