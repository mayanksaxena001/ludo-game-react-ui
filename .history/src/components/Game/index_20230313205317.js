...
function LudoGame(props) {
    ...
    const dicehandler = (data) => {
        if (data) {//data.has_started
            ...
            setGameData(testData);//TODO : not working when on dice 6
    ...
    if (!gameStarted) {
        ...
        return (
            <SocketChannelContext.Provider value={socket}>
                <GameBoard id={gameData.game.id} player={player} playerTurn={playerTurn} handleTokenMove={handleTokenMove} disabled={isDisabled} gameData={gameData} dicehandler={dicehandler} />
