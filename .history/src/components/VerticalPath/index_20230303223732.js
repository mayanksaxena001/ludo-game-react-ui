...
function VerticalPath(props) {
    ...
    const getColor=(boxId)=>{
        COLORED_BOXES_ID.forEach(box_id=>{
            if(box_id===boxId) return color;
        })
        return '';
    }
    const VPath1 = <div className='vpath'>
        <PathBox id='1' color={()=>getColor(id)} value={id} minheight={height} minWidth={width} />
    ...
    const VPath2 = <div className='vpath'>
        <PathBox id='7' color={()=>getColor('7')} value={id} minheight={height} minWidth={width} />
