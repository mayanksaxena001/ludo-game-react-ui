...
function House(props) {
  ...
  const tokenBox = () => {
    let tokens = player.house ? player.house.tokens : [];
    // console.log('tokens',tokens);
    let token1 = tokens ? tokens[0] : null;
    let token2 = tokens ? tokens[1] : null;
    let token3 = tokens ? tokens[2] : {};
    let token4 = tokens ? tokens[3] : {};
    
    return <>
      <div className='tokens'>
        <Box color={color} minheight={height} minWidth={width} tokens={[token1]}/>
        <Box color={color} minheight={height} minWidth={width} tokens={[token2]}/>
      </div>
      <div className='tokens'>
        <Box color={color} minheight={height} minWidth={width} tokens={[token3]}/>
        <Box color={color} minheight={height} minWidth={width} tokens={[token4]}/>
      </div>
    </>
  }
  return (
