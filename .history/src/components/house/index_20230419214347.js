...
function House(props) {
  ...
  const tokenBox = () => {
    ...
    let token1 = tokens && tokens[0] ? [tokens[0]] : [];
    let token2 = tokens && tokens[1] ? [tokens[1]] : [];
    let token3 = tokens && tokens[2] ? [tokens[2]] : [];
    let token4 = tokens && tokens[3] ? [tokens[3]] : [];

    return <>
      <div className='tokens'>
        <Box color={color} height={minHeight} width={minWidth} tokens={token1} handleTokenMove={handleTokenMove} />
        <Box color={color} height={minHeight} width={minWidth} tokens={token2} handleTokenMove={handleTokenMove} />
      </div>
      <div className='tokens'>
        <Box color={color} height={minHeight} width={minWidth} tokens={token3} handleTokenMove={handleTokenMove} />
        <Box color={color} height={minHeight} width={minWidth} tokens={token4} handleTokenMove={handleTokenMove} />
      </div>
    </>
  }
  return (
