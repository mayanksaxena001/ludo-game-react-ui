...
import { connectSocketToServer, disconnectSocket, joinRoom, receiveMessage, resetSocketData, sendMessageToServer, setDiceValue, setGameStarted } from './socketReducer';
...
export const timeOut = (data) => (dispatch) => {
    const socket = io(process.env.REACT_APP_BASE_API_URL); // replace with your own server URL
    if (socket) {
        configSocket(socket, dispatch);
        socket.emit('time_out', data);
        // dispatch(setDiceValue(true));
    }
};
